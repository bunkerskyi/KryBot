					...
					if (xsrf.RestResponse.Content != null)
					{
						...
						if (xsrfToken != null)
						{
							...
							var response = Web.Post(Links.SteamGiftsAjax,
								GenerateJoinParams(xsrfToken.Attributes["value"].Value, "", "sync"),
								headers,
								Cookies.Generate(),
								UserAgent);
							if (response != null)
					...
					if (giveaway.Token != null)
					{
						...
						if (response.RestResponse.Content != null)
						{
							...
							if (jsonresponse.Type == "success")
							{
								...
								task.SetResult(Messages.GiveawayJoined("SteamGifts",
									giveaway.Name,
									giveaway.Price,
									jsonresponse.Points,
									giveaway.Level));
							...
							else
							{
								...
								task.SetResult(Messages.GiveawayNotJoined("SteamGifts",
									giveaway.Name,
			...
			await Task.Run(() =>
				{
					...
					if (response.RestResponse.Content != string.Empty)
					{
							...
							username != null)
						{
							...
							task.SetResult(Messages.ParseProfile("SteamGifts",
								Points,
								Level,
				...
				if (response.RestResponse.Content != string.Empty)
					{
						...
						var gameList = new Dictionary<int, string>();
